...
Game::Game(float mapWidth, float mapHeight, float scale, float speed, size_t maxLength) : mapWidth_(mapWidth), mapHeight_(mapHeight), scale_(scale), speed_(speed), maxLength_(maxLength) {}
...
void Game::Draw(std::chrono::_V2::system_clock::time_point tp)
{
    ...
    for (auto &s : snakes_)
    {
        ...
        DrawLineExRoundEnd({curTail.back().x * (float)scale_, curTail.back().y * (float)scale_}, {newP.x * (float)scale_, newP.y * (float)scale_}, thick, curColor);
        DrawCircle(newP.x * (float)scale_, newP.y * (float)scale_,
                   (float)scale_ / 2, curColor);
    }
}

void Game::SetSnake(int id, Snake snake)
{
    snakes_[id] = snake;
}
...
const Snake &Game::Update(std::chrono::_V2::system_clock::time_point tp)
{
    ...
    // {
    // case up:
    //     s.tail_.back() = {s.tail_.back().x, s.tail_.back().y - dist};
    //     break;
    // case down:
    //     s.tail_.back() = {s.tail_.back().x, s.tail_.back().y + dist};
    //     break;
    // case left:
    //     s.tail_.back() = {s.tail_.back().x - dist, s.tail_.back().y};
    //     break;
    // case right:
    //     s.tail_.back() = {s.tail_.back().x + dist, s.tail_.back().y};
    //     break;
